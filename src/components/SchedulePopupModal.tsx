      ...
      <section role="dialog" aria-modal="true" aria-labelledby="schedule-popup-title" className="flex max-h-[90vh] w-[920px] max-w-full flex-col overflow-hidden rounded-3xl border border-outline-variant bg-surface-container-lowest shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-5">
          <div className="mb-4 flex w-fit rounded-xl bg-surface-container-low p-1">
            <button type="button" aria-pressed={view === 'table'} onClick={() => setView('table')} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold ${view === 'table' ? 'bg-primary text-white shadow-soft' : 'text-on-surface-variant'}`}><Table2 className="h-4 w-4" />표 보기</button>
            <button type="button" aria-pressed={view === 'card'} onClick={() => setView('card')} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold ${view === 'card' ? 'bg-primary text-white shadow-soft' : 'text-on-surface-variant'}`}><LayoutGrid className="h-4 w-4" />카드 보기</button>
          </div>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-grid-line bg-surface-container-low px-5 py-4">
          ...
          <button type="button" onClick={() => onClose(dontShowToday)} className="rounded-xl bg-primary px-5 py-2 text-sm font-bold text-white shadow-soft">닫기</button>
        </footer>
      </section>
    </div>
  );
